...
(function () {
  ...
  function positionAboutLedeCaret() {
    var textEl = document.getElementById("about-lede-typed");
    var caret = document.getElementById("about-lede-caret");
    var p = document.getElementById("about-lede");
    if (!textEl || !caret || !p) return;
    if (!p.classList.contains("lede--typing")) {
      return;
    }
    var pStyle = globalThis.getComputedStyle(p);
    var lineHeightFallback = parseFloat(pStyle.lineHeight);
    var fontSize = parseFloat(pStyle.fontSize);
    if (!Number.isFinite(lineHeightFallback)) {
      lineHeightFallback = Number.isFinite(fontSize) ? fontSize * 1.78 : 18;
    }

    var len = textEl.textContent.length;
    if (len === 0) {
      caret.style.height = Math.max(lineHeightFallback * 0.95, 12) + "px";
      caret.style.left = "0";
      caret.style.top = "0";
      return;
    }
    ...
    var tn = textEl.firstChild;
    if (!tn || tn.nodeType !== Node.TEXT_NODE) {
      textEl.textContent = textEl.textContent;
      tn = textEl.firstChild;
    }
    if (!tn || tn.nodeType !== Node.TEXT_NODE) {
      return;
    }
    var end = Math.min(len, tn.length);
    range.setStart(tn, end);
    range.setEnd(tn, end);
    ...
    var h = rect.height;
    if (!h || h < 4) {
      h = lineHeightFallback;
    }
    caret.style.height = Math.max(h * 0.95, 12) + "px";
    caret.style.left = rect.left - pRect.left + p.scrollLeft + "px";
    caret.style.top = rect.top - pRect.top + p.scrollTop + "px";
  }
  ...
  function renderAboutLedeTypewriter(locale) {
    ...
    var typedEl = document.getElementById("about-lede-typed");
    var suffixEl = document.getElementById("about-lede-suffix");
    ...
    function tick() {
      ...
      suffixEl.textContent = full.slice(i + 1);
      i += 1;
      requestAnimationFrame(positionAboutLedeCaret);
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    window.addEventListener("resize", positionAboutLedeCaret);
